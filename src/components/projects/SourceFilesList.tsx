...
export default function SourceFilesList({ projectId, canManage }: SourceFilesListProps) {
    ...
    const handleDownload = async (fileId: string, fileName: string) => {
        try {
            ...
            if (versions.length === 0) {
                setAlertMessage({ isOpen: true, message: 'Файл не имеет версий' });
                return;
            }
            ...
            setAlertMessage({ isOpen: true, message: err instanceof Error ? err.message : 'Не удалось скачать файл' });
        }
    };
    ...
    const handleDeleteConfirm = async () => {
        ...
        try {
            await api.deleteSourceFile(projectId, deleteConfirm.fileId);
            setDeleteConfirm({ isOpen: false, fileId: null });
            loadFiles();
        } catch (err) {
            setDeleteConfirm({ isOpen: false, fileId: null });
            setAlertMessage({ isOpen: true, message: err instanceof Error ? err.message : 'Не удалось удалить файл' });
        }
    };
    ...
    return (
        <div className="source-files-list-container">
            ...
            <ConfirmDialog
                ...
                message="Удалить файл? Это действие нельзя отменить."
                onConfirm={handleDeleteConfirm}
                onCancel={() => setDeleteConfirm({ isOpen: false, fileId: null })}
                confirmText="Удалить"
                cancelText="Отмена"
